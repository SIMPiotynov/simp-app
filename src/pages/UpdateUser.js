...
export default function UpdateUser() {
  ...
  const loadData = async () => {
    let data = await Promise.all([getUserById(id), fetchAlarms()]).catch((_) => {
      alert("Erreur API");
      return null;
    })
    if(data){
  ...
  return <div className="w-full h-full p-6">
    ...
    <div className="w-full h-full flex items-center justify-center">
      <Formik
          initialValues={{ lastname: user.lastname, firstname: user.firstname, alarmId: user.alarm.id }}
      ...
      >
        <Form className="flex flex-col w-1/2 bg-gray-100 py-8 lg-py-16 px-8 lg-px-40 rounded-xl -mt-40">
          ...
          <div className="flex flex-col">
            ...
            >
              {alarms.map((alarm) => {
                return (
                    <option key={alarm.id} value={alarm.id}>{alarm.name}</option>
                );
              })}
            </Field>
          </div>
          ...
          <p>Ajouté le : {moment.unix(user.createdAt?.timestamp).locale('fr').format('LL')}</p>
          <button type="submit" className="mt-10 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Enregistrer</button>
        </Form>
      </Formik>
    </div>
    )}
